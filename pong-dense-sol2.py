...
def make_network(pixels_num, hidden_units):
  ...
  return pixels, actions, rewards, out, opt, merged,logits,hidden,loss
...
pix_ph, action_ph, reward_ph, out_sym, opt_sym, merged_sym ,logits_sym,hidden_sym,loss_sym= make_network(pixels_num, hidden_units)
...
while True:
  ...
  tf_probs ,tf_logits,tf_hidden= sess.run([out_sym,logits_sym,hidden_sym],feed_dict={pix_ph:x.reshape((-1,x.size))})
  #print(tf_hidden[0])
  y = 1 if np.random.uniform() < tf_probs[0,0] else 0
  sta[int(tf_probs[0,0]*10)]+=1
  #print(tf_probs[0,0])  
  action = 2 + y
  ...
  if done:
    ...
    if episode_number % batch_size == 0:
        ...
        tf_opt, tf_summary ,tf.loss,tf.logits= sess.run([opt_sym, merged_sym,loss_sym,logits_sym], feed_dict={pix_ph:exs,action_ph:eys,reward_ph:ews})
        #for i in range(500):
         #   print(tf.logits[i])
        saver.save(sess, "./log/checkpoints/pg_{}.ckpt".format(step))
        writer.add_summary(tf_summary, step)
        print("datetime: {}, episode: {}, update step: {}, frame size: {}, reward_mean: {} , reward: {} , loss: {}".\
                format(time.strftime('%X %x %Z'), episode_number, step, frame_size, reward_mean,rec,tf.loss))
        print(sta)
        for i in range(11):
            sta[i]=0
